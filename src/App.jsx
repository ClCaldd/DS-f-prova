...
function App() {
  ...
  const getUsers = async () => {
    try {
      const res = await axios.get("ds-f-prova-6usln7xpf-clcaldds-projects.vercel.app/api/person");
      setList(res.data);
  ...
  const registerUser = async () => {
    try { 
      await axios.post("ds-f-prova-6usln7xpf-clcaldds-projects.vercel.app/api/person",{
  ...
  const deleteUser = async (id) => {
    try { 
      await axios.delete(`ds-f-prova-6usln7xpf-clcaldds-projects.vercel.app/api/person/${id}`);
      getUsers()
    } catch (error) {
      console.error("Erro ao inserir", error);
    }
